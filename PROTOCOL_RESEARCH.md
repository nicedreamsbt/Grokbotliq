...
**Shadow liquidator ATAs:** derived via Associated Token Program (`[owner, token_program, mint]`) for `SHADOW_FEE_PAYER` / sim fee payer — no private key. Token program is Tokenkeg or Token-2022 from reserve decode (`token_program_from_mint_owner` helper available). Shadow/planner insert Associated Token Program **CreateIdempotent** (data byte `1`) for missing liquidator ATAs (repay liquidity, withdraw collateral, withdraw liquidity) **after refresh / before flash_borrow & liquidate_v2** so flash `borrowInstructionIndex` stays correct. After CreateIdempotent clears `AccountNotInitialized` (3012), `flash_borrow` marks the repay reserve stale (`last_update.mark_stale`), so liquidate_v2 would hit **Custom 6009 `ReserveStale`** unless we insert post-flash `refresh_reserve`(repay[+withdraw]) + `refresh_obligation` before liquidate (klend-sdk `inBetweenIxs` pattern). `borrowInstructionIndex` is captured at flash_borrow (after ATAs); post-flash refreshes do not shift it. Shadow after this fix: past 6009; next often **Custom 6016 ObligationHealthy** when the HOT/CRITICAL sample is not actually liquidatable on-chain after refresh.

**Live health (candidate selection):** Do **not** use stale obligation SF headers (`unhealthyBorrowValueSf` / `borrowedAssetsMarketValueSf`) for CRITICAL — they are only correct after `refresh_obligation` and previously caused false CRITICAL (~HF 0.998) while on-chain LTV was ~0.33 (Custom 6016 ObligationHealthy). Prefer `compute_obligation_health_live_with_elevation`: deposits/borrows × reserve `marketPriceSf` (U68F60 / 2^60) × effective liq threshold. When `obligation.elevation_group ≠ 0`, use LendingMarket elevation group `liquidationThresholdPct`/`ltvPct` (e.g. group 2 → 92%/87%) and BF=100 — base reserve thr (e.g. 55%) alone false-CRITICAL elevated LST/SOL loops. Liquidatable iff BF-adjusted debt > Σ(deposit_value × liq_threshold_pct/100). Shadow only simulates those; if none in the scan window, report zero + `klend_health_stats` (no pretend CRITICAL).
